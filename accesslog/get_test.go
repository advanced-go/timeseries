...
func ExampleGetByteInvalidArguement() {
	events, status := getByte(nil, "urn:timeseries.access-log.v5", nil)
	fmt.Printf("test: getByte(nil,invalid,nil) -> [status:%v] [err:%v] [events:%v]\n", status, status.Errors(), events)

	//Output:
	//[[] github.com/idiomatic-go/timeseries/accesslog/get [invalid content location: [urn:timeseries.access-log.v5]]]
	//test: getByte(nil,invalid,nil) -> [status:InvalidArgument] [content:invalid content location: [urn:timeseries.access-log.v5]] [events:[]]
